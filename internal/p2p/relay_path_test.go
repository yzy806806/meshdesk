...
func TestAllowedIPsForPeer(t *testing.T) {
	t.Skip("v2: WireGuard-specific test — AllowedIPsForPeer removed")
}
...
func TestRelayPathBuilderNATPeerDiscovered(t *testing.T) {
	...
	if len(circuit.targetEndpoints) != len(natPeer.Endpoints) {
		t.Errorf("circuit targetEndpoints = %v, want %v", circuit.targetEndpoints, natPeer.Endpoints)
	}
}
...
func TestNotifyJoinNATPeerRelayFallback(t *testing.T) {
	...
	// Verify Connect was NOT called (should go through relay path).
	if mockPM.GetConnectCount() != 0 {
		t.Errorf("expected 0 direct peer additions, got %d", mockPM.GetConnectCount())
	}
}

// TestNotifyJoinDirectPeerBypass verifies that peers with endpoints
// are still added directly via Connect, bypassing the relay path builder.
func TestNotifyJoinDirectPeerBypass(t *testing.T) {
	...
	if mockPM.GetConnectCount() != 1 {
		t.Errorf("expected 1 direct peer addition, got %d", mockPM.GetConnectCount())
	}
...
func TestAddRelayTargetAlreadyTracked(t *testing.T) {
	mockPM := newMockPeerManager()
	// Pre-add the peer.
	mockPM.SetConnected("targetkey1234567890abcd", true)

	// Since mockPeerManager.Connect records all calls, we need
	// a different approach. The WireGuardDelegate checks its health map.
	// For the mock, we verify it doesn't error.
	err := mockPM.AddRelayTarget("targetkey1234567890abcd", []string{"10.10.5.5"})
	if err != nil {
