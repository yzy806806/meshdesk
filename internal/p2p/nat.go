...
type NatSession struct {
	...
	// CircuitID is the unique identifier for the relay circuit (if in
	// relay mode). Generated by the entry node and sent to the relay
	// in the circuit_setup message. Used for teardown and health checks.
	CircuitID string
...
type NatTraversal struct {
	...
	// gossipLayer is used to send relay control messages (SETUP, TEARDOWN)
	// via memberlist.SendReliable. May be nil if relay messaging is not
...
// It sends a relay teardown message if the session was using a relay.
func (nt *NatTraversal) RemoveConnection(peerKey string) {
	nt.mu.Lock()
	session, ok := nt.sessions[peerKey]
	nt.mu.Unlock()

	// Send relay teardown if we had a relay circuit.
	if ok && session != nil {
		session.mu.Lock()
		...
		if relayKey != "" && circuitID != "" {
			nt.sendRelayTeardown(relayKey, circuitID)
		}
	}
...
// SetGossipLayer wires the gossip layer so the NAT traversal module
// can send relay control messages (SETUP, TEARDOWN) to relay peers
// via memberlist.SendReliable.
//
// This is called from main.go after both the gossip layer and NAT
// traversal are initialized.
func (nt *NatTraversal) SetGossipLayer(gl *GossipLayer) {
	nt.mu.Lock()
	defer nt.mu.Unlock()
	nt.gossipLayer = gl
	if gl != nil {
		nt.localKey = gl.LocalMeta().PublicKey
	}
}
...
func (nt *NatTraversal) sendRelaySetup(relayKey, targetKey, targetMeshIP string) string {
	...
	if gl == nil {
		return "" // gossip layer not available
	}
	...
	log.Printf("[p2p/nat] sent relay SETUP (circuit %s) to relay %s for target %s",
		circuitID, safeShortKey(relayKey), safeShortKey(targetKey))
	return circuitID
}

// sendRelayTeardown sends a circuit_teardown message to the relay peer
// via gossip, cleaning up the relay session.
func (nt *NatTraversal) sendRelayTeardown(relayKey, circuitID string) {
	nt.mu.RLock()
	gl := nt.gossipLayer
	localKey := nt.localKey
	nt.mu.RUnlock()
	...
	msg := RelayTeardownRequest(localKey, relayKey, circuitID)
	if err := gl.SendRelayMessage(relayKey, msg); err != nil {
		log.Printf("[p2p/nat] failed to send relay TEARDOWN to %s: %v",
			safeShortKey(relayKey), err)
		return
	}

	log.Printf("[p2p/nat] sent relay TEARDOWN (circuit %s) to relay %s",
		circuitID, safeShortKey(relayKey))
}

// generateCircuitID creates a unique 16-byte hex circuit identifier.
