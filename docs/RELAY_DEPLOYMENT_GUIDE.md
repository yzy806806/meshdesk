...
| N1 | 共享节点 (relay) | node1.example.com:22000 (IPv6 SLAAC) | IPv6 only (CGNAT) | `ssh -p 22000 user@node1.example.com` |
...
```bash
# 上传二进制
scp -P 22000 /tmp/meshdesk-arm64 user@node1.example.com:/tmp/meshdesk
ssh -p 22000 user@node1.example.com 'sudo mv /tmp/meshdesk /usr/local/bin/meshdesk'
```
...
# N1 → txcloud (IPv6 直连)
ssh -p 22000 user@node1.example.com \
