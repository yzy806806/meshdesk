...
| 节点 | 角色 | 公网地址 | IP 栈 | SSH |
|------|------|---------|-------|-----|
| 阿里云 | 共享节点 (relay+exit) | 115.29.235.24:52888 | IPv4 only | `ssh -i ~/.ssh/aliyun_key root@115.29.235.24` |
| N1 | 共享节点 (relay) | fn.fxxkccp.top:22000 (IPv6 SLAAC) | IPv6 only (CGNAT) | `ssh -p 22000 yzy806806@fn.fxxkccp.top` |
| txcloud | 普通节点 | IPv6 240d:c000:... | IPv6 (对称NAT, 无入站) | 本机直接操作 |
| Oracle ARM | 普通节点 | 145.241.223.212 + IPv6 | 双栈 (aarch64) | `ssh oracle-arm` |
...
```bash
# 上传二进制
scp -i ~/.ssh/aliyun_key /tmp/meshdesk-amd64 root@115.29.235.24:/usr/local/bin/meshdesk

# SSH 登录后编辑配置
ssh -i ~/.ssh/aliyun_key root@115.29.235.24
```
...
```bash
# 上传二进制
scp -P 22000 /tmp/meshdesk-arm64 yzy806806@fn.fxxkccp.top:/tmp/meshdesk
ssh -p 22000 yzy806806@fn.fxxkccp.top 'sudo mv /tmp/meshdesk /usr/local/bin/meshdesk'
```
...
# N1 → txcloud (IPv6 直连)
ssh -p 22000 yzy806806@fn.fxxkccp.top \
