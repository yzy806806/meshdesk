...
```
共享节点(3个): N1(node1.example.com), 阿里云(固定IP), DS716(example.com)
...
| 格式 | 示例 | 场景 |
|------|------|------|
| **IPv4** | `203.0.113.10:51820` | 固定公网IP节点（如阿里云） |
| **IPv6** | `[240e:360:...]:51820` | IPv6 DDNS节点（如N1 node1.example.com → AAAA） |
| **域名** | `yzy806806.example.com:51820` | DDNS动态域名节点（如DS716） |
...
- 配置文件示例：
  ...
  peers:
    - public_key: "abc..."
      endpoint: "203.0.113.10:51820"        # IPv4
    - public_key: "def..."
      endpoint: "[240e:360:...]:51820"        # IPv6
    - public_key: "ghi..."
      endpoint: "yzy806806.example.com:51820"  # 域名(DDNS)
  ```

**用户的实际部署场景**（参照 EasyTier）：
- N1: `node1.example.com:51820`（域名→IPv6 DDNS，路由器放行端口）
- 阿里云: `203.0.113.10:51820`（固定IPv4）
- DS716: `yzy806806.example.com:51820`（域名→v4+v6 DDNS）
