...
**N1 access:** `ssh -p 22000 user@10.144.144.11`
...
**N1 (arm64, SSH-only):**
- Access via: `ssh -p 22000 user@10.144.144.11`
...
```bash
scp -P 22000 build/meshdesk-arm64 user@10.144.144.11:/tmp/meshdesk
ssh -p 22000 user@10.144.144.11 'sudo cp /tmp/meshdesk /usr/local/bin/meshdesk && sudo chmod +x /usr/local/bin/meshdesk'
scp -P 22000 /path/to/n1-config.yaml user@10.144.144.11:/tmp/config.yaml
ssh -p 22000 user@10.144.144.11 'sudo mkdir -p /etc/meshdesk && sudo cp /tmp/config.yaml /etc/meshdesk/config.yaml'
```
...
```bash
ssh -p 22000 user@10.144.144.11 'sudo kill $(pgrep meshdesk) 2>/dev/null; sleep 1'
ssh -p 22000 user@10.144.144.11 'sudo /usr/local/bin/meshdesk --config /etc/meshdesk/config.yaml 2>&1 | tee /tmp/meshdesk-n1.log &'
```
...
```bash
ssh -p 22000 user@10.144.144.11 'sudo grep "NotifyJoin" /tmp/meshdesk-n1.log | grep "de52c6da"'
```
...
ssh -p 22000 user@10.144.144.11 "sudo grep -E 'gossip layer started|endpoint learning|NotifyJoin|push/pull sync|invalid msgType' \
...
ssh -p 22000 user@10.144.144.11 'sudo ss -tlnp | grep 52888' > "$EVIDENCE_DIR/n1-port-check.txt" 2>/dev/null
...
| N1      | `ssh -p 22000 user@10.144.144.11`                 | `/etc/meshdesk/config.yaml` | `/tmp/meshdesk-n1.log`    |
