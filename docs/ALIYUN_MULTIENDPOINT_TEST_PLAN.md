...
1. Aliyun advertises its sole IPv4 endpoint (`115.29.235.24:52888`) via gossip.
...
|---------|---------------------------|-------------|------------------|------------------------|-------------------|-----------|-----------|
| aliyun  | iZbp10emrt4l28g3ohkq59Z  | amd64       | 10.144.144.10    | 115.29.235.24          | 52888             | :8080     | IPv4 only |
...
│  115.29.235.24   │        │  <ipv6-addr>     │        │  <ipv6-addr>     │
...
**Aliyun (IPv4-only):**
- Verify public IP is reachable: `curl -s https://ifconfig.me` → shows `115.29.235.24`
...
p2p:
  ...
  advertise_endpoints:
    - "115.29.235.24:52888"   # Public IPv4 (reachable by all)
...
This means:
- **Aliyun** advertises `["115.29.235.24:52888"]` — one endpoint, IPv4.
...
Aliyun's `resolveAdvertiseAddr()` picks `115.29.235.24` (the first and only IPv4 endpoint). txcloud's picks `10.144.144.20` (the first IPv4 endpoint in its list). N1's picks `10.144.144.11`.
...
**Expected logs:**
```
[p2p] endpoint learning: announced 1 local endpoint(s): [115.29.235.24:52888] (merged 0 existing)
[p2p] gossip layer started (bind 0.0.0.0:52888, advertise 115.29.235.24)
...
| **ME-2** | Aliyun endpoints propagated | Aliyun node in topology has `endpoints: ["115.29.235.24:52888"]` (non-empty, correct count) | Aliyun endpoints empty or wrong count |
