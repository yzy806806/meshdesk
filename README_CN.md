...
> **当前版本: v2.0.0** —— PunchDataplane raw UDP 数据面：TUN IP 包作为裸 UDP 数据报发送，无 ARQ。实测 31.7 Mbps（首尔↔首尔）和 20.6 Mbps（首尔↔伦敦），对标 EasyTier。路径失效自动降级 relay。Zone 感知打洞、Reality TLS、多路径 relay 路由（Dijkstra），config 12 行，memberlist 退役。 —— UDP 打洞修复（5 个根因：UDPConnFor fallback、double-SERVER 死锁、stale hole endpoint、meta replay、config zone），打洞引擎加固（fd 泄漏、OutboundPort 并发、session 断连清理），UDP stream 泄漏修复，节点重启自动重连，多路径 relay 路由（Dijkstra），config 12 行，memberlist 退役。
